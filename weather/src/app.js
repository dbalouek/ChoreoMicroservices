...
app.get("/api/v1/weather/:zipcode", async (req, res) => {
  const weather = await Weather.find({ zipcode: req.params.zipcode }).sort({date: -1});
  res.json(weather);
});

app.post("/api/v1/weather", async (req, res) => {
  const apiKey = "904eaa61902dba144dc85f29013ec210";
  const url = `https://api.openweathermap.org/data/2.5/weather?zip=${req.body.name},us&units=imperial&appid=${apiKey}`;
